package com.makebk.domain.core.interfaces

import com.makebk.common.annotations.Router
import com.makebk.common.base.ctrl.IBaseController
import com.makebk.domain.core.application.service.ISysGroupResourceService
import com.makebk.domain.core.entits.SysGroupResource

/**
 * <p>
 * 分组资源表 前端控制器
 * </p>
 *
 * @author gme
 * @since 2025-10-02
 */
@Router("/sysGroupResource")
class SysGroupResourceController : IBaseController<ISysGroupResourceService, SysGroupResource>()
